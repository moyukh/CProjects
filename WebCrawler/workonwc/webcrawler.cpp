...
void WebCrawler::crawl()
{
  ...
  while (_headURL < _tailURL && _headURL < _maxUrls)
	...
	free(links);
	  ...
	  while ( word != NULL)
	  {
		  newUrlRecord = new URLRecordList();
		  
		  newUrlRecord -> _urlRecordIndex = _headURL;
		  newUrlRecord -> _next = NULL;
		  
		  
		  if(_wordToURLRecordList -> find(word, urlistptr))
		  {
			 // printf("found\n");
			  //printf("head url %d\n", (*urlistptr) -> _urlRecordIndex);
			  
			  if (_headURL < (*urlistptr) -> _urlRecordIndex) //insert before lesser value
			  {
				  newUrlRecord -> _next = *urlistptr;
				  if (_headURL != (*urlistptr) -> _urlRecordIndex)
					  _wordToURLRecordList -> insertItem(strdup(word), newUrlRecord);
			  }
			  else
			  {
				  
				  URLRecordList* iterator = *urlistptr;
				  while(iterator->_next != NULL && iterator->_next->_urlRecordIndex <= _headURL) //insert after higher value
				  {
					  iterator = iterator->_next;
				  }
				  if(iterator -> _urlRecordIndex != _headURL)
				  {
					  printf("headURL:%d = %d", _headURL, iterator -> _urlRecordIndex);
					  newUrlRecord->_next = iterator->_next;
					  iterator->_next = newUrlRecord;
				  }
		  ...
		  if (_wordToURLRecordList -> find(strdup(word), urlistptr))
		  {
			  temp = *urlistptr;
			  printf("found %s\n", word);
			 // temp = *urlistptr;
			  printf("num %d\n", temp->_urlRecordIndex);
...
void WebCrawler::writeWordFile(const char *wordFileName)
{
	...
	while (iterator.next(word, rec))
	{
		...
		while (iterList != NULL)
		{
			//if(iterList -> _next != NULL)
			//{
			//	if(iterList -> _urlRecordIndex < iterList -> _next -> _urlRecordIndex)
					outFile<<" "<<iterList -> _urlRecordIndex;
		//	}
			iterList = iterList -> _next;
		}
		outFile << "\n";
	}
